...
$(function (){
    //////////////////////////////////////////////////成品合格率//////////////////////////////////////////////////
    let data_cphgl = [17.7,18.0,18.3,18.8,19.1];
    let myChart_cphgl = echarts.init(document.getElementById('cphgl'));
    $.ajax({
        url:'/json/goodsInStoreroomController/getGoodsTop5',
        dataType:'json',
        success:function (ret) {
            ...
            $.each(data,function (index, item) {
                ...
                qualified.push(item.qualifiedPercentRound+"%");
            });
            ...
            myChart_cphgl.setOption(option_cphgl);
        }
    });
...
////////////////////////////////////////////////////原料库存报警//////////////////////////////////////////////////////////
    ...
    let data_ylkcbj = [10,9,8,6,5];
    let myChart_ylkcbj = echarts.init(document.getElementById('ylkcbj'));
    $.ajax({
        url:'/json/material/getMaterialCount',
        dataType:'json',
        success:function (ret) {
            let data = ret;
            let name = [];
            ...
            $.each(data,function (index, item) {
                ...
                overNum.push(item.overNum+item.unit);
            });
            let option_ylkcbj = {
                title:{show:false},
                tooltip: {
            ...
            myChart_ylkcbj.setOption(option_ylkcbj);
        }
    });
...
//////////////////////////////////////////////////////成品库存TOP5/////////////////////////////////////////////////////////
    ...
    let data_cpkc = [15,16,17,18,19];
    let myChart_cpkc = echarts.init(document.getElementById('cpkc'));
    $.ajax({
        url:'/json/goodsInStoreroomController/getCountTop5',
        dataType:'json',
        success:function (ret) {
            // console.log(ret);
            let data = ret;
            ...
            myChart_cpkc.setOption(option_cpkc);
        }
    });


/////////////////////////////////////////////////地图///////////////////////////////////////////////////////////
    let myChart_map = echarts.init(document.getElementById("chinaMap"));
    let geoCoordMap = {
    ...
    let convertData = function (data) {
        ...
        for (let i = 0; i < data.length; i++) {
            for (let geoCoordMapKey in geoCoordMap) {
                if (geoCoordMapKey.indexOf(data[i].city) !== -1 || data[i].city.indexOf(geoCoordMapKey) !== -1){
                    let geoCoord = geoCoordMap[geoCoordMapKey];
                    if (geoCoord) {
                        res.push({
                            name: data[i].city,
                            value: geoCoord.concat(data[i].recentCount,data[i].recentTime,data[i].saleCount)
                        });
                    }
                }
            }
    ...
    $.ajax({
        url:'/json/SalesHistoryRecords/getSalesMessage',
        dataType:'json',
        async:false,
        success:function (ret) {
            // console.log(ret);
            ...
            let symbolSize = [];
            $.each(data,function (index, item) {
                // symbolSize.push(item.saleCount*10);
                symbolSize.push(10);
            });
            ...
            let option = {
                ...
                tooltip : {
                    ...
                    formatter: function(params) {
                        console.log(params);
                        return params.value[2] + ',' + params.value[3] + ',' + params.value[4];
                    }
                },
                geo: {
                    map: 'china',
...
// 首页顶部  产线
    $.ajax({
        url:'/json/produceOrder/getAllLineEfficiency',
        dataType:'json',
        async:false,
        success:function (ret) {
            // console.log(ret)
            ...
            $.each(data,function (index, item) {
                html += "<li class='slide qqq'>";
                html += "<div class='slide_logo'><span>"+item.lineName+"</span> "+ item.lineId+"</div>";
                html += "<div class='data_graph' id='puck"+(index+1)+"'></div>";
                html += "</li>";
            });
            $("#border-definition").html(html);
            ...
            $.each(data,function (index, item) {
                let ch = "puck"+(index +1);
                // 动态创建变量
                eval("myChart_puck" + (index+1).toString() + "=echarts.init(document.getElementById('"+ch+"'))") ;
                let option_puck2 = {
                    title: {
                    ...
                    series: [
                        {
                            ...
                            data:[
                                {value:item.actualAmount, name:'完成'},
                                {value:(item.planAmount-item.actualAmount), name:'未完成'}
                            ]
                        }
                    ],
                    color:['#5de1c3', '#b8fbec'],
                ...
                eval("myChart_puck" + (index+1).toString()).setOption(option_puck2);
            })
        }
    });
...
// 工序监控
    $.ajax({
        url:'/json/OrderDetail/getUnfinishedOrder',
        jsonType:'json',
        async:false,   //同步
        success:function (ret) {

            let html = "";
                // html += "<div class='title'>工序监控</div>";

            $.each(ret,function (index, item) {
                html += "<div class='box_left'><dl>";
                html += "<dd>"+ item.equipmentName +"</dd>";
                html += "<dt>"+ item.unfinishedCount +"</dt>";

                html += "<p>"+ item.processContent +"</p></dl>";
                html += "<div class='arrow_right'></div></div>";
            })
            $(".left").html(html);
        }
    });
    ...
    setTimeout(function (){
        window.onresize = function () {
            myChart_map.resize();
            myChart_cphgl.resize();
            myChart_ylkcbj.resize();
            myChart_cpkc.resize();

            // 产线地方要动态重设大小
            for (let i = 1; i <= len ; i++) {
                eval("myChart_puck" + i.toString()).resize();
            }
        }
    },500);
